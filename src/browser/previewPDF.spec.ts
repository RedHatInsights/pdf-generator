...
const mockConfig = jest.requireMock('../common/config').default;
...
describe('previewPdf', () => {
  ...
  describe('request setup', () => {
    ...
    it('does not forward MOCK_TOKEN in production', async () => {
      const originalMockToken = process.env.MOCK_TOKEN;
      const originalIsProduction = mockConfig.IS_PRODUCTION;
      process.env.MOCK_TOKEN = 'Bearer mock-token';
      ...
      try {
        ...
        mockConfig.IS_PRODUCTION = originalIsProduction;
